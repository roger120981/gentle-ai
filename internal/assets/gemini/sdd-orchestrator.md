...
```
The orchestrator resolves skill paths from the registry ONCE (at session start or first delegation) and passes the exact path to each sub-agent. Sub-agents use the paths provided; if none were provided, they fall back to self-discovery via the skill registry.
